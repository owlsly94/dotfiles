Config
  ...
  , sepChar      = "%"
  , alignSep     = "}{"
  , template     = " <icon=xmonad.xpm/>  %UnsafeStdinReader% }{ %cpu%  | <fc=#7dcfff> %cputemp%</fc>  |  %memory%  |  <fc=#9ece6a>%volume%</fc>  |  <fc=#e0af68>%updates%</fc>  |  %date%  "
  ...
  , commands =
      ...
      , Run Memory
          [ "-t", "<fc=#7aa2f7><fn=1>\61381</fn> <used>Mb</fc>" ] 10
